...
export function GetPresets(config?: any): CompanionPresetDefinitions {
	...
	const inputChannel = (channel: number) => channel - 1
	const mainMix = (mix: number) => mix + 0x2f
	const dca = (dcaNumber: number) => dcaNumber + 0x35
	const muteGroup = (group: number) => group + 0x45
	...
	for (let channel = 1; channel <= inputCount; channel++) {
		...
		addButton(
			...
			`CH ${channel} to 0 dB`,
			`CH ${channel}\n0 dB`,
			[{ actionId: 'fader_input', options: { channel: channelId, level: 0x6b } }],
			BLUE,
		)
		...
		addButton(
			...
			`CH ${channel} to -inf`,
			`CH ${channel}\n-inf`,
			[{ actionId: 'fader_input', options: { channel: channelId, level: 0x00 } }],
			BLACK,
		)
	}
	...
	for (let start = 1; start <= inputCount; start += 8) {
		...
		addButton(
			`unmute_input_bank_${start}_${end}`,
			'Input Bank Mutes',
			`Unmute CH ${start}-${end}`,
			`CH ${start}-${end}\nON`,
			unmuteActions,
	...
	for (let mix = 1; mix <= avantis.mainsCount; mix++) {
		...
		addButton(
			...
			`Main ${mix} to 0 dB`,
			`MAIN ${mix}\n0 dB`,
			[{ actionId: 'fader_master', options: { channel: mixId, level: 0x6b } }],
			BLUE,
		)
	}
	...
	for (let dcaNumber = 1; dcaNumber <= avantis.dcaCount; dcaNumber++) {
		...
		addButton(
			...
			`DCA ${dcaNumber}\n0 dB`,
			[{ actionId: 'fader_DCA', options: { channel: dcaId, level: 0x6b } }],
			BLUE,
		)
	}
	...
	for (let scene = 1; scene <= Math.min(avantis.sceneCount, 100); scene++) {
		...
		addButton(
			`scene_recall_${scene}`,
			`Scenes ${bankStart}-${bankEnd}`,
			`Recall Scene ${scene}`,
			`SCENE\n${scene}`,
			[{ actionId: 'scene_recall', options: { sceneNumber: scene - 1 } }],
			PURPLE,
		)
	}
	...
	for (let aux = 1; aux <= Math.min(avantis.mono.auxCount, 16); aux++) {
		addButton(
			...
			`CH 1 > AUX ${aux}\n0 dB`,
			[{ actionId: 'send_input_to_mono_aux', options: { srcChannel: [0], destChannel: aux - 1, level: 0x6b } }],
			AMBER,
			BLACK,
		)
	}
	...
	addButton(
		...
		'Set CH 1 Name',
		`CH 1\nNAME`,
		[{ actionId: 'channel_name', options: { channel: 0, channelName: 'CH 1' } }],
		BLUE,
	)
	...
	for (const [id, label, colorId, bgcolor] of colors) {
		addButton(
			...
			`Set CH 1 ${label}`,
			`CH 1\n${label.toUpperCase()}`,
			[{ actionId: 'channel_color', options: { channel: 0, color: colorId } }],
			bgcolor,
