...
function default_1(self) {
    const presets = {};
    const config = avantisconfig_json_1.default.config;
    const addButton = (id, category, name, text, actionId, options, bgcolor = BLACK, color = WHITE) => {
        presets[id] = {
            type: 'button',
            category,
            name,
    ...
    for (const [id, label, colorId, bgcolor] of colors) {
        addButton(`channel_1_color_${id}`, 'Channel Name & Color', `Set CH 1 ${label}`, `CH 1\n${label.toUpperCase()}`, 'channel_color', { channel: 0, color: colorId }, bgcolor, id === 'yellow' || id === 'green' ? BLACK : WHITE);
    }
